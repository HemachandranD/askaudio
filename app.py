...
def setup():
    ...
    with left:
        ...
        audio = st.file_uploader(
            "Upload the audio to be analyzed and ask Questions\n",
            type=["mp3", "wav"],
        ...
        with st.form("Launch_form"):
            task = st.radio(
                ...
                options=[
                    "Transcribe :speech_balloon:",
                    "Summary :memo:",
                    "Highlights :chart_with_upwards_trend:",
                    ":rainbow[Custom] :rocket:",
                ],
                horizontal=False,
                captions=[
                    "Get the transcription of the complete Audio. ",
                    "Get the Summary of the Transcribed Audio.",
                    "Get the Highlights of the Audio.",
                    "Ask questions about the Audio and get the Answers from Powerful GPT.",
                ],
            )
...
def launch_task(task, transcript, custom_prompt):
    ...
    if task == "Transcribe :speech_balloon:":
        ...
        with st.spinner("Picking up the Highlights..."):
            highlights_request = (
                "Extract the Highlights from the following" + transcript
            )
            if "highlights" not in st.session_state:
                st.session_state["highlights"] = get_custom_response(
                    request=highlights_request
                )
        ...
        with st.spinner("Generating the Response..."):
            oofContext_prompt = "Answer only from the attached text"
            custom_request = (
                oofContext_prompt + " " + custom_prompt + " " + transcript[:100]
            )
